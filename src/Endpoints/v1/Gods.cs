...
public static class Gods {
    ...
    public static async Task<Results<Ok<List<God>>, BadRequest<string>>> SearchGodsByName(string name, IGodRepository repository, [FromQuery] bool includeAliases = false)
    {
    ...
    public static async Task<Results<Ok<List<God>>, BadRequest<string>>> AddOrUpdateGods(List<GodInput> gods, IGodRepository repository)
    {
        if (gods == null || gods.Count == 0)
            return TypedResults.BadRequest("Gods list must not be empty");
        var result = await repository.AddOrUpdateGods(gods);
        return TypedResults.Ok(result);
    }

    public static async Task<NoContent> DeleteAllGods(IGodRepository repository)
    {
        await repository.DeleteAllGodsAsync();
        return TypedResults.NoContent();
    }

    public static async Task<Results<NoContent, NotFound, BadRequest<string>>> DeleteGodById(int id, IGodRepository repository)
    {
        if (id <= 0)
            return TypedResults.BadRequest("Id must be greater than 0");
        try
